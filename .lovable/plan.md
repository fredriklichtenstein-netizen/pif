## Migration 1 — final, cleaned-up SQL

Simplifications applied:
- `pif_completed` / `wish_completed` branches collapsed to a single unconditional `continue;` with the explanatory comment retained above.
- `wish_archived` per-fulfiller branch comment rewritten to describe the branch as fully silenced (no more "others continue" two-armed framing).

No other changes vs. the previously approved draft.

### File: `db/manual_migrations/wish_aware_reopened_archived_notifications.sql` (replace contents)
...
- Migration 2 (`withdraw_receiver` notification removal) — apply as-is.
- Frontend: remove the `selection_made` self-notification block in `InterestSelectionList.tsx`.
- Backlog: add Site B (owner unselect via `InterestSelectionList.tsx` ~679–715, no notification + no system message) to the missing-signal audit list in `.lovable/plan.md`.
