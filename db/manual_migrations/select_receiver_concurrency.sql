...
-- Ensures only one receiver can be selected per item, even under concurrent calls.
--
-- Strategy:
--   1. Partial unique index on interests(item_id) WHERE status = 'selected'
--      guarantees at most one selected interest per item at the DB level.
--   2. select_receiver RPC locks the item row (FOR UPDATE) so concurrent
--      callers serialize, validates ownership/state, flips the chosen
--      interest to 'selected' and the rest to 'not_selected', and
--      creates/returns the conversation. Re-selecting the same receiver is
--      idempotent; selecting a different one after a winner has been chosen
--      raises a unique-violation error.

CREATE UNIQUE INDEX IF NOT EXISTS interests_one_selected_per_item
