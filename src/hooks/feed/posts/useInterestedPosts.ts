...
export function useInterestedPosts(options: UsePostsBaseOptions = {}) {
  const {
    setPosts,
    setIsLoading,
    setError,
  ...
  const loadInterestedPosts = useCallback(async (currentUser?: User | null) => {
    if (!currentUser) {
      setPosts([]);
      return;
    }
