...
export function usePifCompletion(
  ...
  /**
   * Receiver-side counterpart to withdraw_pif. Resets the receiver's
   * interest, republishes the pif as active for everyone, and closes
   * THIS conversation (conversations.closed_at = now()) so it moves to
   * Historik and becomes read-only for both parties without a refresh.
   */
  const withdrawReceiver = useCallback(
    async (comment?: string) => {
      if (id === null) return { ok: false } as const;
      if (!(await ensureSession("withdraw_receiver"))) return { ok: false } as const;
      const { error } = await (supabase.rpc as any)("withdraw_receiver", {
        p_item_id: id,
        p_comment: comment && comment.trim() ? comment.trim() : null,
      });
      if (error) {
        console.error("withdraw_receiver failed:", error);
        toast({
          title: "Det gick inte att ångra mottagningen",
          description: error.message || "Försök igen.",
          variant: "destructive",
        });
        return { ok: false, error } as const;
      ...
      setState((s) => ({ ...s, pifStatus: "archived" }));
      return { ok: true } as const;
    },
    [id, conversationId],
  );
